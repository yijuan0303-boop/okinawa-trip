...
// Free fallback data. The AI can still use richer context when OPENAI_API_KEY is configured.
const PLACES = [
  ...
  ['瀨長島 Umikaji Terrace',['瀨長島','瀬長島','umikaji','umihotaru'],'豐見城・瀨長島','15:30','約 1.5～2 小時','公車／計程車','靠近機場，適合抵達日或離境日前後。','https://www.umikajiterrace.com/'],
  ...
  ['瀨長島海風露台周邊夕陽',['瀨長島夕陽','瀨長島夕陽','瀨長島海景'],'豐見城・瀨長島','17:00','約 1 小時','公車／計程車','適合傍晚看飛機與海景，和 Umikaji Terrace 可連續安排。','https://www.umikajiterrace.com/'],
  ['豐崎海灘・豊崎美らSUNビーチ',['豐崎海灘','美らSUN海灘','美らSUNビーチ','toyossaki beach'],'豐見城・豐崎','15:00','約 1～1.5 小時','公車／計程車','靠近豐崎商圈，可與 ASHIBINAA、DMM 水族館串接。','https://www.tomigusuku-okinawa.jp/'],
  ...
  ['那霸機場',['那霸機場','沖繩機場','那霸空港','naha airport'],'那霸・機場','08:00','約 1～2 小時','單軌電車／公車／計程車','適合作為抵達或離境節點，不建議當一般觀光景點長時間停留。','https://www.naha-airport.co.jp/']
...
function genericPlace(q){const title=String(q||'沖繩景點').trim();const area=inferArea(title);return {title,area,time:'待確認',stay:'約 1～2 小時',transit:'依目前行程位置選擇單軌電車／公車／計程車',note:`已辨識為「${area}」方向，但目前沒有這個地點的內建詳細資料。可先加入行程，再開 Google Maps 確認實際位置、營業時間與交通；若設定 AI 連線，AI 可進一步補充介紹。`,officialUrl:''};}
...
function fallbackReplan(day,itinerary,newPlace){const list=itinerary.filter(x=>String(x?.day||'')===String(day)).map(placeFromItem);const p=findPlace(newPlace)||genericPlace(newPlace);list.push(p);list.sort((a,b)=>areaRank(a.area)-areaRank(b.area)||timeToMin(a.time)-timeToMin(b.time));let cursor=9*60;const out=list.map(x=>{let t=timeToMin(x.time);if(t===9999||t<cursor)t=cursor;cursor=t+Math.max(45,parseInt(String(x.stay).match(/\d+/)?.[0]||60,10));return toItem(x,day,String(Math.floor(t/60)).padStart(2,'0')+':'+String(t%60).padStart(2,'0'));});return out;}
...
async function openaiPlan(env,mode,place,day,itinerary,trip){
  ...
  const prompt=`你是沖繩旅遊行程規劃助手。請根據使用者目前已安排的行程，新增「${place}」到 ${day}，並依地理區域、既有時間、停留時間與避免來回折返的原則安排。請盡可能提供有用的地點介紹、適合停留時間、區域與一般交通方向。mode=${mode}。若 mode=replan，請重新排序該日全部行程；若 mode=add，仍請在 result 給出合理的新增時間。不要虛構即時公車班次、票價或即時路況；不確定的即時資訊請標示待確認。\n目前行程：${JSON.stringify(itinerary)}\n旅行資訊：${JSON.stringify(trip)}\n請只回傳符合 schema 的 JSON。`;
